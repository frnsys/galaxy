...
from core.ihac2.hierarchy import Hierarchy
...
class ClusteringTest(unittest.TestCase):
    ...
    def test_no_cluster_nodes_with_single_cluster_child(self):
        ...
        self.h.fit(points_1)
        bad_nodes = [n for n in self.h.nodes if self.h.is_cluster(n) and self.h.get_children(n).size <= 1]
...
class HierarchyTest(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(self.h.graph.shape, (3,3))
    ...
    def test_delete_node(self):
        ...
        assert_array_equal(self.h.get_siblings(n), siblings)
        assert_array_equal(self.h.get_parent(n), parent)
        assert_array_equal(self.h.get_children(n), children)

        old_ids = [n] + [c for c in self.h.get_children(n)]

        self.h.delete_node(n)

        # Node should be gone from the hierarchy.
        assert_array_equal(self.h.get_siblings(n), [])
        self.assertEqual(self.h.get_parent(n), None)
        self.assertTrue(n not in self.h.get_children(parent))
        for s in siblings:
            self.assertTrue(n not in self.h.get_siblings(s))
        ...
        for c in children:
            self.assertEqual(self.h.get_siblings(c), [])
            self.assertEqual(self.h.get_parent(c), None)
    ...
    def test_update_distances(self):
        ...
        data = np.insert(data, 0, self.initial_vecs + [self.h.centers[self.initial_clus]], axis=0)
    ...
    def test_get_closest_leaf(self):
        ...
        self.assertEqual(result, self.initial_leaves[0])
    ...
    def test_demote(self):
        ...
        self.assertEqual(n_i, self.h.get_parent(n_j))
    ...
    def test_demote_omits_clusters_with_only_childs(self):
        ...
        self.assertTrue(self.h.is_root(n_i))

        self.assertTrue(n_j in self.h.get_children(n_i))
        self.assertEqual(self.h.get_parent(n_j), n_i)
    ...
    def test_split(self):
        ...
        sibs = self.h.get_siblings(self.initial_leaves[0])
        parent = self.h.get_parent(self.initial_leaves[0])
        ...
        self.assertEqual(self.h.get_siblings(n_i), [n_j, n_k])
        self.assertEqual(self.h.get_siblings(self.initial_leaves[0]), [self.initial_leaves[1], self.h.get_parent(n_i)])
    ...
    def test_restructure(self):
        ...
        self.assertEqual(self.h.get_siblings(self.initial_leaves[0]), [self.initial_leaves[1]])
        ...
        self.assertNotEqual(self.h.get_siblings(self.initial_leaves[0]), [self.initial_leaves[1]])
    ...
    def test_ins_hierarchy(self):
        ...
        n_i = self.h.get_children(self._build_cluster_node())[0]
    ...
    def test_incorporate_adds_to_existing_cluster_node(self):
        ...
        self.assertEqual(self.h.get_siblings(self.initial_leaves[0]), [node_i])
    ...
    def test_incorporate_creates_new_cluster_node(self):
        ...
        self.assertEqual(self.h.get_siblings(self.initial_clus), [node_i])
...
class ClusterNodeTest(unittest.TestCase):
    ...
    def test_init(self):
        c = self.c
        self.assertTrue(self.h.is_cluster(c))
    ...
    def test_split_children(self):
        ...
        children = self.h.get_children(self.c).copy()
        c_i, c_j = self.h.split(self.c)

        expected_i_children = [children[i] for i in [0,1,2]]
        expected_j_children = [children[i] for i in [3,4]]

        assert_array_equal(self.h.get_children(c_i), expected_i_children)
        assert_array_equal(self.h.get_children(c_j), expected_j_children)


class NodeTest(unittest.TestCase):
    """
    Test the hierarchy's handling of node properties.
    """
    ...
    def test_is_cluster(self):
        for clus in self.clusters:
            self.assertTrue(self.h.is_cluster(clus))

        for l in self.leaves:
            self.assertFalse(self.h.is_cluster(l))

    def test_is_root(self):
        clus = self.clusters[0]
        self.assertTrue(self.h.is_root(clus))
        self.assertFalse(self.h.is_root(self.clusters[1]))

        for l in self.leaves:
            self.assertFalse(self.h.is_root(l))

        self.assertEqual(clus, self.h.root)

        # Try making a new root.
        # If the existing root becomes a child, its parent
        # is the new root.
        new_root = self.h.create_node(children=[clus])

        self.assertTrue(self.h.is_root(new_root))
        self.assertFalse(self.h.is_root(clus))
        self.assertEqual(new_root, self.h.root)

    def test_leaves(self):
        assert_array_equal(self.h.leaves, [0,1,3,4])

    def test_nodes(self):
        assert_array_equal(self.h.nodes, [0,1,2,3,4,5])

    def test_get_parent(self):
        assert_array_equal(self.h.get_parent(0), 2)
        assert_array_equal(self.h.get_parent(1), 2)
        assert_array_equal(self.h.get_parent(5), 2)
        assert_array_equal(self.h.get_parent(3), 5)
        assert_array_equal(self.h.get_parent(4), 5)
        assert_array_equal(self.h.get_parent(2), None)

    def test_get_children(self):
        assert_array_equal(self.h.get_children(2), [0,1,5])
        assert_array_equal(self.h.get_children(5), [3,4])

        for l in self.leaves:
            assert_array_equal(self.h.get_children(l), [])

    def test_clear_children(self):
        self.h.clear_children(2)
        assert_array_equal(self.h.get_children(2), [])

    def test_get_siblings(self):
        assert_array_equal(self.h.get_siblings(0), [1,5])
        assert_array_equal(self.h.get_siblings(1), [0,5])
        assert_array_equal(self.h.get_siblings(5), [0,1])
        assert_array_equal(self.h.get_siblings(2), [])
        assert_array_equal(self.h.get_siblings(4), [3])
        assert_array_equal(self.h.get_siblings(3), [4])

    def test_get_leaves(self):
        assert_array_equal(self.h.get_leaves(0), [0])
        assert_array_equal(self.h.get_leaves(1), [1])
        assert_array_equal(self.h.get_leaves(2), [0,1,3,4])
        assert_array_equal(self.h.get_leaves(5), [3,4])
    ...
    def test_add_child(self):
        ...
        self.assertEqual(self.h.get_parent(n), 2)
        self.assertTrue(n in self.h.get_children(2))

        # Changing children should automatically remove it from its previous parent.
        self.h.add_child(5, n)
        self.assertNotEqual(self.h.get_parent(n), 2)
        self.assertEqual(self.h.get_parent(n), 5)
        self.assertFalse(n in self.h.get_children(2))
        self.assertTrue(n in self.h.get_children(5))
